// demo-suport.js — SAAS PROVE "SuportDesk" (i ndare nga Imyr; fshije kur te mbaroje testimi)
...
const CSS = `
  ...
  header{ background:#8e44ad; color:#fff; padding:16px 24px; display:flex; justify-content:space-between; align-items:center; }
  ...
  .cta{ display:inline-block; background:#8e44ad; color:#fff; padding:13px 26px; border-radius:8px;
        text-decoration:none; font-weight:600; border:none; cursor:pointer; font-size:16px; font-family:inherit; }
  .cta:hover{ background:#6f3587; }
  ...
  .feat b{ color:#8e44ad; }
  .rreth{ background:#fff; border:1px solid #e6e8f0; border-left:4px solid #8e44ad; border-radius:10px; padding:20px 22px; margin:8px 0; }
  ...
  .note b{ color:#8e44ad; }
  .ok{ width:56px; height:56px; border-radius:50%; background:#8e44ad22; color:#8e44ad;
  ...
  .steps span.on{ background:#8e44ad; color:#fff; border-color:#8e44ad; }
...
<title>SuportDesk — ${titulli}</title>
<style>${CSS}</style>
</head>
<body>

<header>
  <span class="lg">SuportDesk</span>
  <nav><a href="/demo/suport">Ballina</a><a href="/demo/suport/regjistrohu">Regjistrohu</a></nav>
</header>

${trupi}

<footer>© 2026 SuportDesk — faqe prove per Imyr</footer>

<!-- ═══ NGJIT KETU rreshtin e Imyr-it (copy-paste nga wizard-i) — vlen per TE GJITHA faqet e SuportDesk -->
...
const feats = ["Inbox i unifikuar", "Menaxhim tiketash", "Bazë njohurish (help center)", "Raporte për kohën e përgjigjes"].map(function(v){ return '<div class="feat"><b>&#10003;</b> ' + v + '</div>'; }).join('');

const faqet = {
  ballina: function(){ return layout('ballina', `
<div class="wrap">
  <h1>SuportDesk</h1>
  <p class="lead">Një vend për të gjitha bisedat me klientët tuaj.</p>
  <a class="cta" href="/demo/suport/regjistrohu">Krijo llogari &rarr;</a>

  <section class="rreth">
    <h2>Rreth shërbimit</h2>
    <p>SuportDesk është një help-desk për ekipet e mbështetjes. Mblidhni email, chat dhe mesazhe në një inbox të vetëm, caktoni tiketa te agjentët dhe ndërtoni një bazë njohurish që klientët i gjejnë vetë përgjigjet.</p>
    <div class="kv"><b>Për kë është:</b> Ekipet e mbështetjes së klientit që duan të menaxhojnë kërkesat në një vend dhe të reduktojnë kohën e përgjigjes.</div>
    <div class="kv"><b>Fjalë-kyçe:</b> mbështetje, tiketa, help desk, chat, klientë</div>
  </section>
...
<div class="wrap">
  ...
  <div class="card">
    ...
    <div style="margin-top:20px;">
      <button class="cta" onclick="location.href='/demo/suport/welcome'">Regjistrohu &rarr;</button>
    </div>
  </div>
  <div class="note">Pas butonit kalon te <b>/demo/suport/welcome</b>.</div>
</div>`); },
...
<div class="wrap">
  ...
  <div class="ok">&#10003;</div>
  <h1>Mire se erdhe te SuportDesk!</h1>
